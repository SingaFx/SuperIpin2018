...
LOTS = cfgData.LOTS
RISK_param = cfgData.risks
minLOT = cfgData.min_lot
lot2usd_ratio_param = cfgData.lot2usd_ratio
...
class FormWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.tbl_viewResult.setRowCount(len(pairs))
    ...
    def chk_mqlVer(self):
        ...
        for slave in slave_brokers:
            if slave.zmq_mt4_ver == zmq_ver_cfg:
                print(slave.company, '-', slave.acctName[:6], 'Zmq version check Pass')
            else:
                print(slave.company, '-', slave.acctName[:6], 'Zmq version check Failed')
                ...
                if preCheck and not noTradeTime and slave.balance <= max_profit:               
                    ...
                    # to close position if price converge again
                        ...
                        if slave.order_type[i] == BUY and slave.pos[i] < slave.arbitrage_close and \
                            ...
                            print (slave.company, slave.acctName[:10], ': Close order')
                            ...
                            data = [slave.tms, slave.company, slave.acctName[:10], slave.symbols[i], slave.trade_count[i], slave.bid[i], 'CLOSE']
                            write_2_file(data)
                            ...
                            data = [slave.tms, slave.company, slave.acctName[:10], slave.symbols[i], slave.trade_count[i], slave.ask[i], 'CLOSE']
                            write_2_file(data)
                        ...
                        # open layer if price gap further
                            ...
                            print (slave.company, slave.acctName[:10], ': Open order buy for next lvl ', slave.symbols[i])
                            ...
                            slave.send_order(SELL, slave.symbols[i], slave.bid[i], slave.lots, slave.SLIP, slave.stop_loss, mt4_comments)  # IC Market
                            print (slave.company, slave.acctName[:10], ': Open order sell for next lvl ', slave.symbols[i])
...
#        symbol_index = None
        ...
        master_broker.order_close_new(selSymbols1)
        slave_brokers[self.broker_index].order_close_new(selSymbols2)
...
class broker_class:
    ...
    def __init__(self, broker, magic_no, symb, risk='manual', ratio=1, offsets=None):       
...
for i in range(len(slave_ips)):
    ip = 'tcp://' + slave_ips[i]
    slave_brokers.append(broker_class(ip, magic_number, slave_symbols[i], \
    RISK_param[i], lot2usd_ratio_param[i], gaps_offsets[i]))
