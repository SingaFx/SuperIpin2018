...
app_name = 'SuperIpin 10.5d'
ea_name_ver = 'SuperIpin 10.5d'
zmq_ver_cfg = 'zeromq_11.0'
cfgData = ReadConfig.config('superipin_old.cfg')
...
class FormWidget(QWidget):
    ...
    def init_var(self):
        ...
        # assign variable as list
        self.cnt_arb = []

        # iterate based on how many pair traded
        for i in range(len(pairs)):
            # append the variable cnt_arb
    ...
    def run_1Leg_arb(self):
        ...
        #get prices for broker 1 and broker 2
        broker1.get_price(symbols1)
        broker2.get_price(symbols2)

        #get order status for broker 1 and broker 2
        broker1.get_order_status(symbols1)
        broker2.get_order_status(symbols2)

        # get acct info for broker 1 and broker 2
        broker1.get_acct_info()
        broker2.get_acct_info()

        #calculate gap arbitrage to get Pos and Neg
        self.cal_gapPosNeg()

        #calculate count arbitrage
        self.cal_countArbitrage()

        # calculate and assign lots depends on the config file
        ...
        for i in range(len(symbols1)):
            ...
            if broker1.tms == broker2.tms and broker1.bid[i] != 0 and broker2.bid[i] != 0 and \
                broker1.connection and broker2.connection:

                preCheck = True
                #save the data into database, timestamp is from MT4
                if saveData:
            ...
            if preCheck and not noTradeTime and acct_balance <= max_profit:
                ...
                if broker2.trade_count[i] == 0:
                    ...
                    if self.cnt_arb[i] >= cnt_arb_limit:

                        #send BUY order
                        broker2.send_order(BUY, symbols2[i], broker2.ask[i], lots, SLIP, stop_loss, mt4_comments)
                        ...
                        # broker2_ask, 'tradeType', 'level', 'posNeg']
                        data = [broker2.tms, symbols2[i], broker2.trade_count[i], broker2.ask[i], broker1.bid[i] \
                                , broker1.ask[i], broker2.bid[i], broker2.ask[i], 'BUY', 'level1', self.pos[i] \
                                , self.cnt_arb[i]]
                        write_2_file(data)

                    #part 2 : if cnt arbitrage lesser than -cnt_arb_limit
                    elif self.cnt_arb[i] <= - cnt_arb_limit:
                        ...
                        # ['timestamp', 'symbol', 'countTrade', entry_price, broker1_bid, broker1_ask, broker2_bid, \
                        # broker2_ask, 'tradeType', 'level', 'posNeg']
                        data = [broker2.tms, symbols2[i], broker2.trade_count[i], broker2.bid[i], broker1.bid[i] \
                                , broker1.ask[i], broker2.bid[i], broker2.ask[i], 'SELL', 'level1', self.neg[i] \
                                , self.cnt_arb[i]]
                        write_2_file(data)
                ...
                # to close position if price converge again
                    ...
                    if broker2.order_type[i] == BUY and self.pos[i] < arbitrage_close and \
                        self.chk_closeValid(broker2, symbols2[i]):
                    ...
                    elif broker2.order_type[i] == SELL and self.neg[i] > -arbitrage_close and \
                        self.chk_closeValid(broker2, symbols2[i]):
                    ...
                    elif broker2.order_type[i] == BUY and self.pos[i] >= next_step and self.cnt_arb[i] >= cnt_arb_limit:
                    ...
                    elif broker2.order_type[i] == SELL and self.neg[i] <= - next_step and self.cnt_arb[i] <= -cnt_arb_limit:
        ...
        for i in range(len(pairs)):
            ...
            if broker1.tms == broker2.tms and broker1.bid[i] != 0 and broker2.bid[i] != 0 and \
                ...
                if self.pos[i] >= arbitrage_open:
                    self.cnt_arb[i] = self.cnt_arb[i] + 1
                ...
                elif self.neg[i] > -arbitrage_open or self.pos[i] < arbitrage_open:
                    self.cnt_arb[i] = 0
...
class TestWidget(QWidget):
    def __init__(self, parent):
        super(TestWidget, self).__init__(parent=None)
...
superIpin = MyMainWindow(symbols1,symbols2)
superIpin.show()
sys.exit(app.exec_())
